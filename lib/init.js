 ...
 */
exports.checkCondition = function (callback) {
    console.log('请确认你处于公司内网且已经准入：');
    require('read')(
        { prompt: 'y/n > ' },
        function (err, answer) {
            answer.toLowerCase() = answer.toLowerCase();
...
function getPackageVersions() {
    ...
    u.each(
        config.packages,
        function (package) {
            var version = package.location.split('/')[2];
...
exports.renderProjectTemplate = function (callback) {
    ...
    function writeAsCopy(source) {
        ...
        if (stat.isFile()) {
            ...
            var content = engine.compile(template)(templateData);
