...
var EDP_PACKAGES = [ 'ub-ria', 'esui-flat-theme', 'echarts', 'er-track', 'est', 'uioc' ];
var EMPTY_USER_INFO = { name: 'Unknown', email: 'unknown@unknown.com' };
var EXAMPLE_DICT = {
    channel: '频道',
    slot: '广告位',
    delivery: '广告',
...
exports.initializeProjectInfo = function (callback) {
    ...
    require('async').series(
        [
            function (callback) {
                console.log('请输入此项目的名称（中文，如“广告联盟”）：');
                require('read')(
                    { prompt: '> ' },
                    function (err, name) {
            ...
            function (callback) {
                console.log('请输入此项目的代号（通常为英文，如“ADUnion”）：');
                require('read')(
                    { prompt: '> ' },
                    function (err, alias) {
            ...
            function (callback) {
                console.log('请输入本项目的系统域名（不包含http://前缀，如“unknown.unknown.com”）：');
                require('read')(
                    { prompt: '> ' },
                    function (err, domain) {
                        projectInfo.domain = domain || 'unknown.unknown.com';
                        callback(err);
                    }
                );
            },
...
exports.renderProjectTemplate = function (callback) {
    ...
    var templateData = {
        ...
        flatThemeVersion: packageVersions['esui-flat-theme']
    };
    ...
    function writeAsCopy(source) {
        ...
        var isTplFile = source.search(/\.tpl\.html/) >=0;
        ...
        if (stat.isFile()) {
            ...
            if (!isTplFile) {
                content = template ? engine.compile(template)(templateData) : '';
            }
...
exports.initializeModuleInfo = function (callback) {
    var projectDirectory = require('./util').findProjectDirectory();
    var projectInfo = {
        dir: projectDirectory,
        infoDir: path.join(projectDirectory, '.edpproj')
    };
    // 将新引入的包信息更新到module.conf文件中
    require('edp-project').module.updateConfig(projectInfo);
    console.log('完成包信息到module.conf更新');
    callback();
};
...
exports.updateModuleConfig = function (callback) {
    ...
    fs.writeFile(
        ...
        function (err) {
            ...
            // 将module.conf文件的config配置分发到各个html文件中
